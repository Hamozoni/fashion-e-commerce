...
function ImagesGalary() {
    ...
    const {product,selectedColor} = useContext(ProductDetailsContext)
    const [selectedImage,setSelectedImage] = useState(product.images[selectedColor][0]?.imagePath);

    useEffect(()=> {

        setSelectedImage(product.images[selectedColor][0]?.imagePath);
